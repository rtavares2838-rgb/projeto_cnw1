...
if ($isLoggedIn && isset($_SESSION['id'])) {
    ...
    // Inclui a conexão (supondo que 'conexao/conecta.php' define $conn)
...
<script>
    ...
    async function deletarNotificacaoAndRefreshDropdown(id) {
        ...
        try {
            // 1. Deleta no servidor (Requer o arquivo deletar_notificacao.php)
            const res = await fetch('deletar_notificacao.php', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ id })
            });
    ...
    async function updateNotificationDropdownContent() {
        ...
        try {
            // Assume que 'buscar_notificacoes.php' está no mesmo nível ou acessível
            ...
            lista.innerHTML = `<p class="cs-no-notifications" style="color:var(--bad);">Erro ao carregar. Detalhes: ${e.message}</p>`;
        }
    }
